...
Not we need something on disk, like a database. I know two ways of implementing these workers,
...
To connect with postgres we need a running postgres server, you can run it with docker or use a manged postgres service like [Neon](https://neon.tech). In any case you will have a **Database Connection String** which looks like this **"postgres://username:password@host:port/database"**
...
```go showLineNumbers{42} title="main.go" {3,14,22,23,46}
[...]
...
If you find the solution of the problem then don't let me know. (_kunal [at] kunalsin9h [dot] com_)
