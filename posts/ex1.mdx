---
title: Elixir 1, Genera language features and variables
...
Today we are going to start a tour of elixir language, these are my notes while learning elixir from the book _"Elixir in Action, Second Edition"_ by _Sasa Juric_. I have alwasy used conventional note book for writing technical notes, mainly notes when learning new programming language and i think they are not that useful, hence this is me trying to take notes on [Obsidian](https://obsidian.md) and later publish on my [blog](https://kunalsin9h.com/blog).

> **Note**: This is not a guide, nor a tutorial, it just just which i may have written in my note book, so these are think form the language which i kept for future reference, i.e this series will mostly contain `nontrivial` concepts and constructs.
...
Rebinding doesn’t mutate the existing memory location. It
...
> I have mixed feeling about this, as language like `Rust` handle memory with garbage collector, which increases the performance of the language.
